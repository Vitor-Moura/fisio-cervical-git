...
class ArduinoDataRead {
    ...
    //função para gerar dado aleatório
    fake_data(){
        setInterval(() => {
    ...
    SetConnection() {
        SerialPort.list().then(listSerialDevices => {
            ...
            parser.on('data', (data) => {
                // split é o que faz a fragmentação para separar por virgula e transormar em vetor para jogar dentro do value
				let value = data.toString().split(';');
                // variável de temperatura na posição 1 e 
                // replace para onde estiver /r (que vem do arduino) ser subtituido por nada(espaço em branco)
				let temperature = parseFloat(value[1].replace('\r', ''));
                // variável de umidade na posição 0
				let humidity = parseFloat(value[0].replace('\r', ''));
                // populando a lista umidade e temperatura
